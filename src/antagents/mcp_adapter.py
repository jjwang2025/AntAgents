"""This module implements the antagents adapter.

antagents do not support async tools, so this adapter will only work with the sync
context manager.

To enable structured output support (MCP spec 2025-06-18+), use:
AntAgentsAdapter(structured_output=True)

Example Usage:
>>> with MCPAdapt(StdioServerParameters(command="uv", args=["run", "src/echo.py"]), AntAgentsAdapter()) as tools:
>>>     print(tools)
"""
...
import mcp
import antagents  # type: ignore
from antagents.utils import _is_package_available  # type: ignore
...
class AntAgentsAdapter(ToolAdapter):
    """Adapter for the `antagents` framework.

    Note that the `antagents` framework do not support async tools at this time so we
    ...
    def __init__(self, structured_output: bool = False):
        """Initialize the AntAgentsAdapter.
    ...
    def adapt(
        ...
        """Adapt a MCP tool to a antagents tool.
        ...
        # antagents will handle type detection at runtime
        output_type = "object"

        class MCPAdaptTool(antagents.Tool):
            def __init__(
                self,
                name: str,
                description: str,
                ...
                if isinstance(content_item, mcp.types.TextContent):
                    ...
                    if not _is_package_available("torchaudio"):
                        raise ValueError(
                            ...
                            "Please install it with `uv add mcpadapt[antagents,audio]`.",
                        )
                    else:
    ...
    async def async_adapt(
        ...
        raise NotImplementedError("async is not supported by the antagents framework.")
