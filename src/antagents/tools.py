...
class Tool(BaseTool):
    ...
    def save(self, output_dir: str | Path, tool_file_name: str = "tool", make_gradio_app: bool = True):
        """
        保存工具的相关代码文件以便推送到 Hub。这会将工具的代码复制到 `output_dir` 并自动生成：

        - 一个 `{tool_file_name}.py` 文件，包含工具的逻辑。
        如果传递 `make_gradio_app=True`，还会写入：
        - 一个 `app.py` 文件，当工具通过 `tool.push_to_hub()` 导出到空间时，为其提供 UI
        - 一个 `requirements.txt` 包含工具使用的模块名称（通过检查其代码检测到）

        参数:
            output_dir (`str` 或 `Path`): 要保存工具的文件夹。
            tool_file_name (`str`, *可选*): 保存工具的文件名。
            make_gradio_app (`bool`, *可选*, 默认为 True): 是否还导出 `requirements.txt` 文件和 Gradio UI。
        """
        # 确保输出目录存在
    ...
    def push_to_hub(
        ...
        参数:
            repo_id (`str`):
                要推送工具的仓库 ID。推送到特定组织时应包含组织名称。
            commit_message (`str`, *可选*, 默认为 `"Upload tool"`):
                推送时的提交消息。
            private (`bool`, *可选*):
                是否将仓库设为私有。如果为 `None`（默认），除非组织的默认设置为私有，否则仓库将为公开。如果仓库已存在，则忽略此值。
            token (`bool` 或 `str`, *可选*):
                用于远程文件的 HTTP 承载授权的令牌。如果未设置，将使用运行 `huggingface-cli login` 时生成的令牌（存储在 `~/.huggingface` 中）。
    ...
    def _initialize_hub_repo(repo_id: str, token: bool | str | None, private: bool | None) -> str:
        ...
        metadata_update(repo_url.repo_id, {"tags": ["antagents", "tool"]}, repo_type="space", token=token)
    ...
    def from_space(
        ...
        参数:
            ...
            description (`str`):
                工具的描述。
            api_name (`str`, *可选*):
                如果空间有多个选项卡，则使用特定的 api_name。如果未指定，将默认为第一个可用的 api。
            token (`str`, *可选*):
        ...
        class SpaceToolWrapper(Tool):
            ...
            def __init__(
                ...
                # 如果未定义 api_name，则为此空间取第一个可用的 API
                if api_name is None:
...
class ToolCollection:
    ...
    def from_mcp(
        ...
        ```py
        >>> import os
        >>> from antagents import ToolCollection, CodeAgent, InferenceClientModel
