...
import pathvalidate
import requests
from serpapi import GoogleSearch # 注意需要导入的是 pip install google-search-results
...
class SimpleTextBrowser:
    """(预览版) 一个极其简单的基于文本的网页浏览器，类似于Lynx。适用于代理使用。"""
    ...
    def __init__(
        ...
        self.viewport_size = viewport_size  # 仅适用于标准uri类型
        ...
        self._find_on_page_last_result: int | None = None  # 上一次结果的定位位置
    ...
    def find_on_page(self, query: str) -> str | None:
        """从当前视口开始向前搜索查询字符串，必要时循环回到开头。"""

        # 我们是通过之前相同查询的find_on_page搜索到达这里的吗？
        # 如果是，映射到find_next
    ...
    def _find_next_viewport(self, query: str, starting_viewport: int) -> int | None:
        ...
        nquery = nquery.replace(" __STAR__ ", "__STAR__ ")  # 将孤立的星号与前一个单词合并
